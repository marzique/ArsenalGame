...
class Control:
	...
	def move_player_pos(self, mult, rot_disp):
	    x = self.player.pos[0] + math.sin(self.rad(self.player.rot + rot_disp)) * SPEED * mult
	    y = self.player.pos[1] + math.cos(self.rad(self.player.rot + rot_disp)) * SPEED * mult
	    self.player.pos = (x, y)
	    ...
	    while self.running:
	        for event in pygame.event.get():
	            ...
	            ## Get the change in x position of the mouse since last check, and change the angle by it
	                ...
	                self.player.rot += x_angular_movement * MOUSE_SENSITIVITY
	        ...
	        if keys[pygame.K_UP] or keys[pygame.K_w]:
	            self.move_player_pos(1, 0)
	        if keys[pygame.K_DOWN] or keys[pygame.K_s]:
	            self.move_player_pos(-1, 0)
	        if keys[pygame.K_d]:
	            self.move_player_pos(1, 90)
	        if keys[pygame.K_a]:
	            self.move_player_pos(-1, 90)
	        ...
	        for wall in WALLS:
	            ...
	            px1 = (x1 - self.player.pos[0]) / TOP_ZOOM
	            py1 = (y1 - self.player.pos[1]) / TOP_ZOOM
	            px2 = (x2 - self.player.pos[0]) / TOP_ZOOM
	            py2 = (y2 - self.player.pos[1]) / TOP_ZOOM

	            # Wall positions relative to player's position and rotation
	            rx1 = math.cos(self.rad(-self.player.rot)) * px1 + math.sin(self.rad(-self.player.rot)) * py1
	            ry1 = math.cos(self.rad(-self.player.rot)) * py1 - math.sin(self.rad(-self.player.rot)) * px1
	            rx2 = math.cos(self.rad(-self.player.rot)) * px2 + math.sin(self.rad(-self.player.rot)) * py2
	            ry2 = math.cos(self.rad(-self.player.rot)) * py2 - math.sin(self.rad(-self.player.rot)) * px2
	        ...
	        pygame.draw.line(split_screen, PLAYER_RAY_COLOR,
	            ...
	            self.screen_coords(0, RAY_LENGTH), 1)
	        ...
	        sorted_walls = self.sort_walls(WALLS)
